...
interface SwipeableWorkerTabNavigatorProps {
  tabs: Tab[];
  initialTab?: number;
}

// Navigation context for programmatic navigation
const WorkerTabNavigationContext = createContext<{
  navigateToTab: (tabKey: string, params?: any) => void;
} | null>(null);

export const useWorkerTabNavigation = () => {
  const context = useContext(WorkerTabNavigationContext);
  if (!context) {
    throw new Error('useWorkerTabNavigation must be used within SwipeableWorkerTabNavigator');
  }
  return context;
};

// Ref interface for external navigation
export interface SwipeableWorkerTabNavigatorRef {
  navigateToTab: (tabKey: string, params?: any) => void;
}

const SwipeableWorkerTabNavigator = forwardRef<SwipeableWorkerTabNavigatorRef, SwipeableWorkerTabNavigatorProps>(({
  tabs,
  initialTab = 0,
}, ref) => {
  ...
  return (
    <WorkerTabNavigationContext.Provider value={{ navigateToTab }}>
    ...
    </WorkerTabNavigationContext.Provider>
  );
});
...
export default SwipeableWorkerTabNavigator;
